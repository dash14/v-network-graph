...
function a2c(
  ...
  if (!recursive) {
    ...
    var rx2 = rx * rx,
      ...
      f1 = Math.asin(toFloat(((y1 - cy) / ry).toFixed(9))),
      f2 = Math.asin(toFloat(((y2 - cy) / ry).toFixed(9)))
...
function parsePathString(pathString: string) {
  ...
  const paramCounts: Record<string, number> = {
    ...
    data: any[] = []
  if (!data.length) {
    pathString.replace(pathCommand, function (a: string, b: string, c: string) {
      let params: any[] = [],
        name = b.toLowerCase()
...
function path2curve(path: string): PathSegments {
  let p = pathToAbsolute(path),
    ...
    processPath = function (path: Segment, d: Attrs, pcom: string): Segment {
      let nx,
        ny,
        tq = { T: 1, Q: 1 }
      if (!path) {
    ...
    fixArc = function (pp: PathSegments, i: number) {
      if (pp[i].length > 7) {
        pp[i].shift()
        const pi = pp[i]
        while (pi.length) {
    ...
    pfirst: string = "", // temporary holder for original path command
    pcom: string = "" // holder for previous path command of original path

  for (let i = 0, ii = mmax(p.length, 0); i < ii; i++) {
    ...
    attrs.bx = toFloat(seg[seglen - 4] as string) || attrs.x
    attrs.by = toFloat(seg[seglen - 3] as string) || attrs.y
  }
  return p
}
...
function pathLengthFactory(isTotal: number = 0, subpath: number = 0) {
  return function (
    ...
    let x: number,
      ...
      point: Dots,
      len = 0
    for (let i = 0, ii = curve.length; i < ii; i++) {
      p = curve[i] as [string, ...number[]]
      if (p[0] == "M") {
        x = +p[1]
        y = +p[2]
      } else {
...
const getTotalLength = pathLengthFactory(1)
const getPointAtLength = pathLengthFactory()
const getSubpathsAtLength = pathLengthFactory(0, 1)

export function parsePathD(d: string): PathSegments {
  return path2curve(d)
}

export function calculateSubpath(path: string, from: number, to: number) {
  if (from < 0) from = 0
  if (to < 0) to = 0
  if (from === 0 && to === 0) {
    return path
  ...
  /* @ts-ignore */
  const total = getTotalLength(path) as number
  to = total - to
  if (total - to < 1e-6) {
    /* @ts-ignore */
